...
def run_gradcam_all(
        ...
        device: torch.device,
        output_dir: str,
        num_images: Optional[int] = 15,
        alpha: float = 0.2,
        tasks: Optional[set[Literal["nature", "materiality", "biological", "landscape-type"]]] = None
):
    """
    ...
    if not tasks:
        tasks = ["nature", "materiality", "biological", "landscape-type"]
    ...
    for task_name in tasks:
        ...
        print(f"\n→ Generando Grad-CAM para '{task_name}' → {task_dir}")

        # Envolver el modelo para que devuelva solo la salida task_idx
        single_model = SingleOutputWrapper(multi_model, output_index=task_dict[task_name])
        ...
        for img_idx in range(len(test_dataset)):
            if num_images and saved >= num_images:
                break
            ...
            # 5) Create side-by-side image (double width)
            # Convert BGR to RGB for proper display (cv2 uses BGR)
            orig_rgb = cv2.cvtColor(orig_uint8, cv2.COLOR_BGR2RGB)
            over_rgb = cv2.cvtColor(over, cv2.COLOR_BGR2RGB)

            # Concatenate horizontally (side by side)
            side_by_side = np.concatenate([orig_rgb, over_rgb], axis=1)

            # Convert back to BGR for cv2.imwrite
            side_by_side_bgr = cv2.cvtColor(side_by_side, cv2.COLOR_RGB2BGR)

            out_path = os.path.join(task_dir, f"{img_name}_{task_name}_comparison.png")
