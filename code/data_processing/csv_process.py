...
def process_csv3(file_path, img_dir, valid_extensions=['.jpg', '.png', '.jpeg']):
    df = pd.read_csv(file_path)
    new_rows = []
    for idx in range(4):
        nature_col = f'nature_visual_{idx}'
        material_col = f'nep_materiality_visual_{idx}'
        biological_col = f'nep_biological_visual_{idx}'
        landscape_col = f'landscape-type_visual_{idx}'
        
        temp_df = df[['platform_id', nature_col, material_col, biological_col, landscape_col]].copy()
        temp_df['index'] = idx
        temp_df = temp_df.rename(columns={
            nature_col: 'nature_visual',
            material_col: 'nep_materiality_visual',
            biological_col: 'nep_biological_visual',
            landscape_col: 'landscape-type_visual'
        })
